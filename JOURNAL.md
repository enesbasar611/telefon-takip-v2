...
- [x] 2026-05-15: WhatsApp stabilizasyonu ve profil isim fallback sistemi güncellendi. Dosyalar: `src/lib/whatsapp/whatsapp-manager.ts`, `src/lib/auth.ts`, `src/app/api/whatsapp/config/route.ts`. Neden: WhatsApp başlatma sürecine 45s zaman aşımı ve 'force' re-init desteği eklendi. Google ile girişlerde ismin '...' gelmesi sorunu email prefix fallback ile giderildi. Servis kaydı oluşturma sürecine detaylı server-side loglar eklenerek canlı ortam takibi güçlendirildi. Komut: `npx tsc --noEmit`. Sonuç: Başarılı.
