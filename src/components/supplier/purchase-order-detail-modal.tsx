                    ...
                    <div className="p-8 space-y-8 a4-content print:p-0 w-full" id="print-area">
                        <DialogHeader className="flex flex-row items-center justify-between no-print border-b border-white/5 pb-6">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <DialogTitle className="text-2xl font-black tracking-tight">Sipariş Detayı / Fiş</DialogTitle>
                                    <p className="text-xs font-medium text-muted-foreground pt-1">Sipariş No: #{order.orderNo}</p>
                                </div>
                            </div>
                            <Button onClick={handlePrint} variant="default" size="lg" className="h-12 px-8 rounded-2xl gap-3 font-black bg-blue-600 hover:bg-blue-700 text-white shadow-xl shadow-blue-500/20">
                        ...
                        <div className="hidden print:flex justify-between border-b-2 border-black pb-4 mb-8">
                            <div>
                                <h1 className="text-3xl font-black tracking-tighter mb-1">SATIN ALMA FORMU</h1>
                                <div className="flex items-center gap-2 text-sm text-slate-500">
                                    <span className="font-black text-black">NO: #{order.orderNo}</span>
                                    <span>•</span>
                                    <span>{format(new Date(order.createdAt), "dd MMMM yyyy HH:mm", { locale: tr })}</span>
                                </div>
                            </div>
                            <div className="text-right">
                                <h2 className="text-xl font-black uppercase tracking-widest text-blue-600">TAKIPV2</h2>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 print:grid-cols-2 print:gap-10">
                            <div className="p-6 rounded-3xl bg-white/[0.02] border border-white/5 print:p-0 print:border-none">
                                <div className="flex items-center gap-2 mb-4 text-blue-400 print:text-black">
                                    ...
                                    <h4 className="text-[10px] font-black uppercase tracking-widest">Tedarikçi Bilgileri</h4>
                                </div>
                                <div className="space-y-1">
                                    <p className="text-lg font-black text-foreground print:text-xl print:text-black">{order.supplier?.name || "Bilinmeyen Tedarikçi"}</p>
                            ...
                            <div className="p-6 rounded-3xl bg-white/[0.02] border border-white/5 print:p-0 print:border-none">
                                <div className="flex items-center gap-2 mb-4 text-purple-400 print:text-black">
                                    ...
                                    <h4 className="text-[10px] font-black uppercase tracking-widest">Sipariş Durumu</h4>
                                </div>
                                <div className="space-y-3">
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs font-bold text-muted-foreground print:text-black text-[10px] uppercase">Tarih:</span>
                                        <span className="text-xs font-black text-foreground print:text-black">{format(new Date(order.createdAt), "dd.MM.yyyy HH:mm", { locale: tr })}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs font-bold text-muted-foreground print:text-black text-[10px] uppercase">Durum:</span>
                                        <Badge className="font-black bg-blue-500/10 text-blue-400 border-none print:text-black print:bg-transparent px-0 uppercase">
                        ...
                        <div className="space-y-4 print:mt-10">
                            <div className="flex items-center justify-between no-print">
                                <h3 className="text-sm font-black text-foreground uppercase tracking-widest">Sipariş İçeriği</h3>
                            </div>
                            <div className="rounded-3xl border border-white/5 bg-white/[0.01] overflow-hidden print:rounded-none print:border-none">
                                <table className="w-full text-left print-table">
                                    <thead className="bg-white/[0.04] border-b border-white/5 print:bg-slate-100">
                                        <tr>
                                            <th className="px-6 py-4 text-[10px] font-black text-muted-foreground uppercase tracking-wider print:text-black">Ürün Adı</th>
                                            <th className="px-6 py-4 text-[10px] font-black text-muted-foreground uppercase tracking-wider text-center print:text-black">Talep</th>
                                            <th className="px-6 py-4 text-[10px] font-black text-muted-foreground uppercase tracking-wider text-center print:text-black">Alınan</th>
                                            <th className="px-6 py-4 text-[10px] font-black text-muted-foreground uppercase tracking-wider text-right print:text-black">Birim</th>
                                            <th className="px-6 py-4 text-[10px] font-black text-muted-foreground uppercase tracking-wider text-right print:text-black">Toplam</th>
                                        </tr>
                                    </thead>
                                        ...
                                        {(order.items || []).map((item: any) => (
                                            <tr key={item.id} className="hover:bg-white/[0.02] transition-colors">
                                                ...
                                                <td className="px-6 py-5 text-center">
                                                    <span className="text-sm font-black text-foreground print:text-black">{item.quantity}</span>
                                                </td>
                                                <td className="px-6 py-5 text-center">
                                                    <span className="text-sm font-black text-emerald-400 print:text-black">{item.receivedQuantity || 0}</span>
                                                </td>
                                                ...
                                                <td className="px-6 py-5 text-right font-black text-sm print:text-black">
                        ...
                        <div className="flex flex-col items-end pt-6 border-t border-white/5 print:border-black/20">
                            <div className="w-80 space-y-3">
                                <div className="flex items-center justify-between text-muted-foreground print:text-black">
                                    ...
                                    <span className="text-sm font-black">₺{(Number(order.totalAmount) || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })}</span>
                                </div>
                                <div className="flex items-center justify-between text-muted-foreground print:text-black">
                                ...
                                <div className="flex items-center justify-between pt-3 border-t border-white/10 print:border-black">
                                    <span className="text-sm font-black uppercase text-foreground print:text-black">Genel Toplam</span>
                                    <span className="text-3xl font-black text-blue-500 tracking-tighter print:text-black">₺{(Number(order.totalAmount) || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="hidden print:flex print:mt-20 print:justify-between px-10">
                            <div className="text-center">
                                ...
                                <p className="text-[10px] font-black uppercase">Teslim Eden</p>
                            </div>
                            <div className="text-center">
                                <div className="w-40 border-b border-black mb-2"></div>
                                <p className="text-[10px] font-black uppercase">Teslim Alan</p>
                            </div>
                        </div>
                    </div>
                </div>
