        ...
        <div className="flex flex-col gap-2">
            {addedTo.length > 0 && (
                <div className="flex flex-wrap gap-1">
                    {addedTo.map((name) => (
                        <Badge key={name} className="bg-emerald-500/10 text-emerald-400 border-none text-[9px] font-black px-2 py-0.5 gap-1">
            ...
            {!open ? (
                ...
                <div className="flex items-center gap-2 p-2 rounded-xl bg-white/[0.03] border border-white/10 flex-wrap">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <span className="text-xs font-black text-foreground w-6 text-center">{qty}</span>
                        <Button
                            variant="ghost"
                            size="icon"
                            className="h-7 w-7 rounded-lg bg-white/5 border border-white/10 text-muted-foreground hover:bg-white/10"
                    ...
                    <Button
                        ...
                        onClick={handleAdd}
                        className="h-8 px-3 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-black text-[10px]"
                    >
                ...
                <div className="relative p-8 bg-gradient-to-br from-blue-600/10 to-transparent border-b border-white/5 overflow-hidden shrink-0">
                    ...
                    <div className="relative z-10 flex items-start gap-4">
                        ...
                        <div className="flex-1">
                            <Badge className="bg-blue-600/10 text-blue-500 border-none text-[9px] font-black uppercase tracking-[0.2em] px-3 mb-2">
                            ...
                            <p className="text-sm text-slate-400 font-medium mt-1">
                                Her ürüne <span className="text-blue-400 font-black">+</span> basarak tedarikçi seç ve sipariş listene ekle
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="relative z-10 flex gap-3 mt-5 flex-wrap">
                        <div className="flex items-center gap-2 bg-rose-500/10 border border-rose-500/20 rounded-xl px-4 py-2">
                            ...
                            <span className="text-xs font-black text-rose-400">{totalUrgent} Acil Ürün</span>
                        </div>
                        <div className="flex items-center gap-2 bg-blue-500/10 border border-blue-500/20 rounded-xl px-4 py-2">
                            <TrendingUp className="h-3.5 w-3.5 text-blue-400" />
                            <span className="text-xs font-black text-blue-400">{trendingAlerts.length} Trend Ürün</span>
                        </div>
                        <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-4 py-2">
                            <ShoppingCart className="h-3.5 w-3.5 text-slate-400" />
                            <span className="text-xs font-black text-slate-400">{suppliers.length} Tedarikçi</span>
                        </div>
                    </div>
                </div>
                                    ...
                                    <div key={alert.id} className={cn("flex items-start gap-4 p-5 rounded-2xl border", style.bg, style.border)}>
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 mb-1">
                                                <Badge className={cn("text-[8px] font-black border-none px-2 py-0.5", style.bg, style.color)}>{style.label}</Badge>
                                                {alert.product && (
                        ...
                        <div className="space-y-3">
                                ...
                                <div key={p.id} className={cn(
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2">
                                            <Badge className={cn("text-[8px] font-black border-none px-2 py-0.5", p.stock <= 0 ? "bg-rose-500/10 text-rose-400" : "bg-amber-500/10 text-amber-400")}>
                                        ...
                                        <p className="text-[10px] text-slate-500 font-medium">
                                            Mevcut: <span className={cn("font-black", p.stock <= 0 ? "text-rose-400" : "text-amber-400")}>{p.stock}</span> / Kritik Seviye: {p.criticalStock}
                                        </p>
                                    </div>
                                    <SupplierPickerRow
                            ...
                            {additionalCritical.length > 5 && !showAll && (
                                <button
                                    ...
                                    className="w-full flex items-center justify-center gap-2 py-3 text-xs font-black text-slate-500 hover:text-blue-400 transition-colors"
                                >
                            ...
                            {trendingAlerts.map((alert) => {
                                ...
                                return (
                                    <div key={alert.id} className={cn("flex items-start gap-4 p-4 rounded-2xl border", style.bg, style.border)}>
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <Badge className={cn("text-[8px] font-black border-none px-2 py-0.5 mb-1", style.bg, style.color)}>{style.label}</Badge>
                    ...
                    {alerts.length === 0 && additionalCritical.length === 0 && (
                        <div className="flex flex-col items-center justify-center py-16 gap-4">
                            ...
                            <div className="text-center">
                                <h3 className="font-black text-slate-500 uppercase text-sm">Her Şey Yolunda</h3>
