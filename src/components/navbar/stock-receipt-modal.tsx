        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-md bg-card border-none p-0 overflow-hidden shadow-2xl">
                <DialogHeader className="p-8 bg-blue-500/10 border-b border-blue-500/10 flex flex-col items-center text-center">
                    ...
                    <DialogTitle className="text-2xl font-black text-white tracking-tight">EKSİK LİSTESİ</DialogTitle>
                    <p className="text-[10px] font-black text-blue-400 mt-2 tracking-[0.2em] uppercase">TEDARİK VE SATIN ALMA FORMU</p>
                </DialogHeader>
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="bg-muted/30 p-4 rounded-2xl border border-white/5 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-[10px] font-black text-slate-500 uppercase">OLUŞTURULMA TARİHİ</span>
                        </div>
                    ...
                    <div className="receipt-preview bg-white text-black p-6 rounded-xl border border-slate-200 font-mono text-[9px] leading-relaxed shadow-inner">
                        <div className="text-center border-b border-black border-dashed pb-3 mb-3">
                            <h3 className="font-bold text-xs uppercase italic">{settings?.title || "BAŞAR TEKNİK"}</h3>
                            <p className="font-bold text-[8px] mt-0.5">{settings?.subtitle || "EKSİK ÜRÜN & TEDARİK LİSTESİ"}</p>
                            <p className="text-[8px] mt-1 italic">Tel: {settings?.phone}</p>
                        </div>
                            ...
                            <div className="space-y-1">
                                {items.map((item, idx) => (
                                    <div key={idx} className="grid grid-cols-12 gap-2 py-0.5 border-b border-black/5 last:border-0">
                                        <span className="col-span-8 truncate font-bold uppercase">{item.name}</span>
                        ...
                        <div className="text-center mt-6 pt-3 border-t border-black border-dashed opacity-70">
                            <p className="font-bold">{settings?.footer || "Tedarik Listesi Otomatik Oluşturulmuştur"}</p>
                ...
                <DialogFooter className="p-6 bg-slate-900/50 border-t border-white/5 gap-3">
                    <Button variant="ghost" onClick={onClose} className="h-12 rounded-xl font-bold text-slate-500 hover:text-white hover:bg-white/5">Kapat</Button>
                    <Button onClick={handlePrint} className="flex-1 h-12 rounded-xl bg-blue-500 hover:bg-blue-400 text-black font-black gap-2 shadow-lg shadow-blue-500/20 active:scale-95 transition-all">
