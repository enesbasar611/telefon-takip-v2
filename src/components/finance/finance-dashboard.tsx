...
export function FinanceDashboard({ summary }: { summary: any }) {
    ...
    const displayPrefix = displayCurrency === "USD" ? "$" : displayCurrency === "EUR" ? "€" : "₺";
    const toDisplayAmount = (value: number) => {
        if (displayCurrency === "USD") return value / usdRate;
        if (displayCurrency === "EUR") return value / eurRate;
        return value;
    };
    ...
    const bugunKar = summary?.todayProfit || 0;
    const bugunGider = summary?.todayExpense || 0;

    const cards = [
        { label: "BUGÜNKÜ GELİR", value: bugunGelir, icon: TrendingUp, color: "text-emerald-500", bg: "bg-emerald-500/10", description: "Bugün yapılan peşin satış toplamı" },
        { label: "BUGÜNKÜ KÂR", value: bugunKar, icon: Receipt, color: "text-cyan-500", bg: "bg-cyan-500/10", description: "Satış fiyatı - ürün maliyeti" },
                    ...
                    <Card key={i} className="border-zinc-200 dark:border-zinc-800 shadow-sm group overflow-hidden relative bg-card/50 backdrop-blur-sm rounded-[2.5rem]">
                        ...
                        <CardContent className="p-8">
                            ...
                            <div className="mt-1 flex items-baseline gap-2">
                                <RevealFinancial amount={toDisplayAmount(stat.value)} prefix={displayPrefix} className="text-4xl tracking-tighter" />
                            </div>
