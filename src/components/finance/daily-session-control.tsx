                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                        ...
                        <Dialog open={openModal} onOpenChange={setOpenModal}>
                            <DialogTrigger asChild>
                                <Button className="rounded-xl shadow-lg shadow-blue-500/10 h-10 px-6 text-xs font-bold gap-2">
                                ...
                                <form onSubmit={handleOpenSession} className="p-10">
                                    ...
                                    <div className="space-y-6">
                                        <div className="space-y-2">
                                            ...
                                            <Input id="openingBalance" name="openingBalance" type="number" required defaultValue="0" step="0.01" className="h-11 rounded-xl text-sm font-bold bg-muted/20 border-border/40" />
                                        </div>
                                        <div className="space-y-2">
                                    ...
                                    <div className="mt-8 flex gap-3">
                                        ...
                                        <Button type="submit" disabled={loading} className="flex-[2] h-11 text-xs font-bold rounded-2xl shadow-lg shadow-blue-500/20 bg-blue-600 hover:bg-blue-700 text-white">
            ...
            <CardContent className="p-6">
                <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                    ...
                    <Dialog open={closeModal} onOpenChange={setCloseModal}>
                        <DialogTrigger asChild>
                            <Button variant="outline" className="rounded-xl border-emerald-500/30 text-emerald-500 hover:bg-emerald-500 hover:text-white h-10 px-6 text-xs font-bold gap-2 transition-all">
                        ...
                        <DialogContent className="sm:max-w-lg border-border/40 p-0 overflow-hidden bg-background/95 backdrop-blur-xl rounded-[2rem] shadow-2xl max-h-[90vh] flex flex-col">
                            ...
                            <form onSubmit={handleCloseSession} className="p-8 flex flex-col overflow-hidden">
                                <DialogHeader className="mb-6 p-0 shrink-0">
                                    ...
                                    <DialogTitle className="text-xl font-bold">Kasa Oturumunu Kapat</DialogTitle>
                                    ...
                                    <div className="grid grid-cols-2 gap-3 mb-2">
                                        <div className="bg-emerald-500/5 p-4 rounded-2xl border border-emerald-500/10">
                                            ...
                                            <div className="mt-2 flex flex-col gap-1">
                                                ...
                                                <span className="text-[9px] font-bold text-muted-foreground opacity-60">Havale: ₺{sessionSummary.bankIn.toLocaleString('tr-TR')}</span>
                                            </div>
                                        </div>
                                    ...
                                    <div className="bg-muted/30 p-4 rounded-2xl border border-border/40">
                                        ...
                                        <div className="space-y-2 max-h-[120px] overflow-y-auto pr-2 custom-scrollbar">
                                            {session.transactions?.map((t: any) => (
                                                <div key={t.id} className="flex items-center justify-between group">
                                                    <div className="flex items-center gap-2 overflow-hidden">
                                                        ...
                                                        <p className="text-[10px] font-bold truncate opacity-80">{t.description}</p>
                                                    </div>
                                            ...
                                            {(!session.transactions || session.transactions.length === 0) && (
                                                <p className="text-[10px] text-muted-foreground font-bold text-center py-4 opacity-50">Henüz hareket kaydı yok.</p>
                                            )}
                                        </div>
                                    </div>
                                ...
                                <div className="mt-4 pt-4 border-t border-border/40 flex gap-3 shrink-0">
                                    <Button type="button" variant="ghost" onClick={() => setCloseModal(false)} className="flex-1 h-12 text-xs font-bold rounded-2xl uppercase tracking-widest">İPTAL</Button>
                                    <Button type="submit" disabled={loading} className="flex-[2] h-12 text-xs font-bold rounded-2xl bg-rose-600 hover:bg-rose-700 text-white shadow-xl shadow-rose-500/20 uppercase tracking-widest">
