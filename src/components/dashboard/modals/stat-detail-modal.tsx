                        ...
                        <div className="flex items-center justify-between">
                            ...
                            {!selectedSupplier ? (
                                <Link href={config.route}>
                                    <Button variant="outline" className="rounded-xl h-9 text-[10px] font-black uppercase tracking-widest px-4 border-border/40 hover:bg-muted transition-all">
                        ...
                        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                            <div className="p-6 rounded-3xl bg-indigo-500/5 border border-indigo-500/10">
                                <div className="flex justify-between items-center mb-4">
                                    <span className="text-[10px] font-black text-muted-foreground uppercase opacity-60">TOPLAM GÜNCEL BORÇ</span>
                                    <span className="text-xl font-black text-indigo-500 tracking-tight">₺{Number(selectedSupplier.balance).toLocaleString('tr-TR')}</span>
                                </div>
                                <div className="space-y-4 pt-4 border-t border-indigo-500/10">
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">ÖDEME YAPILACAK HESAP</label>
                                        <div className="grid grid-cols-2 gap-2">
                                            {accounts.map((acc) => (
                                                ...
                                                >
                                                    <div className={cn("text-[10px] font-black uppercase tracking-tighter mb-1", selectedAccountId === acc.id ? "text-white" : "text-foreground")}>{acc.name}</div>
                                                    <div className={cn("text-xs font-black", selectedAccountId === acc.id ? "text-indigo-100" : "text-emerald-500")}>₺{Number(acc.balance).toLocaleString('tr-TR')}</div>
                                                    {selectedAccountId === acc.id && (
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">ÖDEME TUTARI (₺)</label>
                                        <input
                                            type="number"
                                            value={paymentAmount}
                                            onChange={(e) => setPaymentAmount(e.target.value)}
                                            placeholder="Tutar giriniz..."
                                            className="w-full h-12 bg-background border border-border/40 rounded-xl px-4 text-sm font-black focus:border-indigo-500 focus:outline-none transition-all"
                                        />
                                    </div>

                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">AÇIKLAMA (OPSİYONEL)</label>
                                        <input
                                            type="text"
                            ...
                            <Button
                                ...
                                className="w-full h-14 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-xs font-black uppercase tracking-widest shadow-xl shadow-indigo-600/30 transition-all active:scale-[0.98]"
                            >
                        ...
                        <ScrollArea className="h-[400px] pr-4">
                            {loading ? (
                                <div className="flex flex-col items-center justify-center h-full gap-4 text-muted-foreground">
                                    ...
                                    <p className="text-xs font-black uppercase tracking-[0.2em] animate-pulse">Veriler Getiriliyor...</p>
                                </div>
                            ) : data.length === 0 ? (
                                <div className="flex flex-col items-center justify-center h-full gap-4 text-muted-foreground/40 text-center uppercase">
                                    ...
                                    <p className="text-xs font-black tracking-widest">Henüz bir kayıt bulunamadı</p>
                                </div>
                            ) : (
                                                ...
                                                <div key={supplier.id} className="p-5 rounded-[1.5rem] bg-muted/20 border border-border/40 group hover:border-indigo-500/30 transition-all">
                                                    <div className="flex items-center justify-between mb-4">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div>
                                                                <h4 className="text-sm font-black uppercase tracking-tight">{supplier.name}</h4>
                                                                <p className="text-[10px] font-bold text-muted-foreground opacity-60 uppercase">{supplier.phone || "Telefon Yok"}</p>
                                                            </div>
                                                        </div>
                                                        <div className="text-right">
                                                            <p className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-widest mb-1">BEKLEYEN BORÇ</p>
                                                            <p className="text-lg font-black text-indigo-500 tracking-tight">₺{Number(supplier.balance).toLocaleString('tr-TR')}</p>
                                                        </div>
                                                    </div>
                                                    <Button
                                                        onClick={() => {
                                                        ...
                                                        className="w-full h-11 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-[10px] font-black uppercase tracking-widest shadow-lg shadow-indigo-600/20"
                                                    >
                                            ...
                                            {data.map((item: any) => (
                                                <div key={item.id} className="p-4 rounded-2xl bg-muted/20 border border-border/10 flex items-center justify-between group hover:bg-background/50 transition-all">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div>
                                                            <h4 className="text-sm font-black uppercase tracking-tight">{item.sale?.customer?.name || "Hızlı Satış"}</h4>
                                                            <p className="text-[10px] font-black text-muted-foreground opacity-60 uppercase tracking-tighter">
                                                                {item.createdAt ? format(new Date(item.createdAt), "HH:mm", { locale: tr }) : "00:00"} • {item.account?.name || "Kasa"}
                                                            </p>
                                                        </div>
                                                    </div>
                                                    <div className="text-right">
                                                        <p className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-widest mb-1">TUTAR</p>
                                                        <p className="text-base font-black text-emerald-500 tracking-tight">₺{Number(item.amount).toLocaleString('tr-TR')}</p>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                                    ...
                                    {type === "DAILY_SALES" && statsData && (
                                        <div className="grid grid-cols-3 gap-3 mb-6">
                                            <div className="bg-muted/10 border border-border/40 rounded-2xl p-4 flex flex-col items-center justify-center text-center">
                                                <span className="text-[9px] font-black text-muted-foreground uppercase tracking-widest mb-1 opacity-70">KASA AÇILIŞ</span>
                                                <span className="text-sm font-black text-foreground">{statsData.kasaOpeningBalance || "₺0"}</span>
                                            </div>
                                            <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-4 flex flex-col items-center justify-center text-center">
                                                <span className="text-[9px] font-black text-emerald-600/70 uppercase tracking-widest mb-1">GÜNLÜK SATIŞ</span>
                                                <span className="text-sm font-black text-emerald-500">+{statsData.todaySales || "₺0"}</span>
                                            </div>
                                            <div className="bg-primary/10 border border-primary/20 rounded-2xl p-4 flex flex-col items-center justify-center text-center">
                                                <span className="text-[9px] font-black text-primary/70 uppercase tracking-widest mb-1">GÜNCEL KASA</span>
                                                <span className="text-sm font-black text-primary">{statsData.kasaBalance || "₺0"}</span>
                                            </div>
                                        </div>
                                    )}
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-4">
                                                            ...
                                                            <div className="min-w-0">
                                                                <h4 className="text-[13px] font-black uppercase tracking-tight line-clamp-1">{String(title || "Bilinmiyor")}</h4>
                                                                <p className="text-[10px] font-black text-muted-foreground opacity-60 uppercase tracking-tighter truncate mt-0.5">
