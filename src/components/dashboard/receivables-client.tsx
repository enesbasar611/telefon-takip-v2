...
import { TrendingDown, Users, ChevronRight, ArrowLeft, Wallet, Calendar, Search, X as CloseIcon, MessageCircle } from "lucide-react";
...
export function ReceivablesClient({ debts }: ReceivablesClientProps) {
    ...
    const openWhatsAppModal = (name: string, phone: string, totalTRY: number, totalUSD: number) => {
        ...
        let msg = `Merhaba ${name},\n\nBekleyen ödemeniz hakkında bilgi alabilir miyim?\n\n`;
        if (totalTRY > 0) msg += `Güncel Borç (TL): ${formatCurrency(totalTRY)}\n`;
            ...
            <CardHeader className="flex-shrink-0 flex flex-row items-center justify-between border-b border-border/40 p-8 pb-6">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col">
                        <CardTitle className="font-black text-xl tracking-tighter uppercase text-orange-950/80">
                            {selectedCustomerId ? "Borç Detayları" : "Alacaklarım"}
                        </CardTitle>
                        <div className="flex items-center gap-2 mt-0.5">
                            <div className="h-1.5 w-1.5 rounded-full bg-orange-500 animate-pulse" />
                            <p className="text-[10px] text-orange-900/40 uppercase font-black tracking-widest leading-none">
                ...
                {!selectedCustomerId ? (
                    ...
                    <div className="flex flex-col h-full animate-in fade-in slide-in-from-right-4 duration-500">
                        ...
                        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                            <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-4 px-2">HİSSETİĞİMİZ İŞLEMLER</p>
                        ...
                        <Button
                            className="w-full mt-6 h-14 rounded-[1.5rem] font-bold text-sm tracking-tight uppercase"
                            variant="secondary"
