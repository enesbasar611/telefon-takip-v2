      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button variant="ghost" size="sm" className="h-7 text-[9px] font-black uppercase bg-cyan-500/10 text-cyan-500 border border-cyan-500/20 rounded-lg hover:bg-cyan-500/20 px-3">
          ...
          <DialogContent className="bg-[#141416] border-white/5 text-white">
            ...
            <div className="grid gap-4 py-4">
              ...
              <Button onClick={handleAdd} disabled={isPending || !selectedProductId} className="bg-cyan-500 text-black font-black uppercase h-12 rounded-xl mt-4">
      ...
      <div className="space-y-2">
        {currentParts.map((part) => (
          <div key={part.id} className="flex items-center justify-between p-3 rounded-xl bg-white/[0.02] border border-white/[0.03] group hover:border-white/10 transition-all">
            ...
            <span className="text-xs font-black text-cyan-500">₺{(Number(part.unitPrice) * part.quantity).toLocaleString('tr-TR')}</span>
          </div>
        ))}
