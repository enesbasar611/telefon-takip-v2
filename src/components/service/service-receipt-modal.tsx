        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-md bg-card border-none p-0 overflow-hidden shadow-2xl">
                <DialogHeader className="p-8 bg-blue-500/10 border-b border-blue-500/10 flex flex-col items-center text-center relative overflow-hidden">
                    ...
                    <DialogTitle className="text-2xl font-black text-white tracking-tight leading-none z-10 italic">SERVİS FİŞİ ÖNİZLEME</DialogTitle>
                    <p className="text-[10px] font-black text-blue-400 mt-2 tracking-[0.3em] uppercase z-10">{ticket.ticketNumber} HAZIRLANDI</p>
                </DialogHeader>

                <div className="p-8 space-y-6">
                    {/* Receipt Preview (Thermal Layout) */}
                    <div className="receipt-preview bg-white text-black p-8 rounded-2xl shadow-inner border border-slate-200 font-mono text-[10px] leading-snug">
                        <div className="text-center border-b-2 border-black pb-4 mb-4">
                            <h3 className="font-black text-sm uppercase italic tracking-tighter">{settings?.title || "BAŞAR TEKNİK"}</h3>
                        ...
                        <div className="flex justify-between items-center bg-black text-white px-2 py-1.5 mb-4">
                            <span className="font-black text-[9px] tracking-widest">KAYIT NO</span>
                            <span className="text-xs font-black">{ticket.ticketNumber}</span>
                        </div>

                        <div className="grid grid-cols-2 gap-2 mb-4 border-b border-black border-dashed pb-3">
                            <div>
                                <p className="text-[7px] font-black text-gray-500 mb-0.5">Müşteri</p>
                                <p className="font-black text-[10px]">{ticket.customer?.name}</p>
                                <p className="text-[8px] font-bold mt-0.5">{ticket.customer?.phone}</p>
                            </div>
                            <div className="text-right">
                                <p className="text-[7px] font-black text-gray-500 mb-0.5">Tarih</p>
                        ...
                        <div className="mb-4 space-y-2">
                            <div className="bg-gray-100 px-2 py-1 mb-2 font-black text-[8px] text-center tracking-tighter shadow-sm border border-gray-200">CİHAZ VE ARIZA BİLGİLERİ</div>
                            <div className="flex justify-between border-b border-gray-100 pb-1">
                                <span className="text-gray-500 text-[8px] font-bold">MARKA / MODEL:</span>
                                <span className="font-black">{ticket.deviceBrand} {ticket.deviceModel}</span>
                            </div>
                            <div className="flex justify-between border-b border-gray-100 pb-1">
                            ...
                            <div className="mt-2">
                                ...
                                <p className="bg-gray-50 p-2 border border-gray-200 rounded italic text-[9px] font-medium leading-tight select-all">"{ticket.problemDesc}"</p>
                            </div>
                        </div>
                        ...
                        {settings?.terms && (
                            <div className="bg-gray-50 p-3 rounded-lg border border-gray-200 mb-6">
                                <p className="text-[8px] font-black mb-1.5 border-b border-gray-200 pb-1 uppercase tracking-tighter">Önemli Şartlar</p>
                        ...
                        <div className="flex justify-between border-t-2 border-black pt-3 items-center">
                            <span className="font-black text-[9px]">TAHMİNİ ÜCRET:</span>
                            <span className="text-lg font-black">₺{Number(ticket.estimatedCost).toLocaleString('tr-TR')}</span>
                        </div>
                        ...
                        <div className="text-center mt-6 pt-4 border-t border-black border-dashed opacity-70">
                            <p className="font-black text-[9px]">{settings?.footer || "Cihazınız güvenli ellerde."}</p>
                ...
                <DialogFooter className="p-6 bg-slate-900/50 border-t border-white/5 gap-3">
                    ...
                    <Button onClick={handlePrint} className="flex-1 h-14 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white font-black gap-3 shadow-xl shadow-blue-500/20 active:scale-95 transition-all">
