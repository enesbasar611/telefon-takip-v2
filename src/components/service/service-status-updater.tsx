...
const statusMap: Record<ServiceStatus, { label: string; color: string }> = {
  ...
  DELIVERED: { label: "TESLİM EDİLDİ", color: "bg-cyan-500" },
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button disabled={isPending} className="w-full h-12 rounded-2xl bg-cyan-500 text-black font-black uppercase tracking-widest hover:bg-cyan-400 shadow-cyan-strong transition-all flex items-center justify-center gap-2">
          {isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : "DURUMU GÜNCELLE"}
          {!isPending && <ChevronDown className="h-4 w-4" />}
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="bg-[#141416] border-white/5 text-white p-2 min-w-[220px] shadow-2xl">
        <DropdownMenuLabel className="text-[10px] uppercase tracking-[0.2em] font-black text-gray-500 p-3">Yeni Durum Seçin</DropdownMenuLabel>
