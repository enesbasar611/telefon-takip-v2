    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="w-full md:max-w-4xl h-full md:h-auto md:max-h-[85vh] overflow-hidden bg-[#0B0F19] border-border text-foreground/90 p-0 md:rounded-3xl flex flex-col">
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col h-full overflow-hidden">
          <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-border/60 p-5 md:p-8 gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <DialogTitle className="font-medium text-xl md:text-2xl text-white">Cihazı Düzenle</DialogTitle>
                <p className="text-[10px] md:text-sm text-muted-foreground/80 font-medium">{device.name} detaylarını güncelleyin.</p>
              </div>
            </div>

            <div className="flex md:hidden flex-col gap-2">
              <Label className="text-[10px] text-muted-foreground uppercase tracking-widest pl-1">Cihaz Durumu</Label>
              <Select value={condition} onValueChange={(val: Condition) => setValue("condition", val)}>
                <SelectTrigger className="h-12 bg-card border-border rounded-xl">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="bg-[#0B0F19] border-border">
                  <SelectItem value="NEW">SIFIR</SelectItem>
                  <SelectItem value="USED">2. EL</SelectItem>
                  <SelectItem value="INTERNATIONAL">YURTDIŞI</SelectItem>
                </SelectContent>
              </Select>
            </div>

            <div className="hidden md:flex bg-background p-1.5 rounded-2xl border border-border/50 shadow-inner">
          ...
          <div className="flex-1 overflow-y-auto p-5 md:p-8 space-y-8 custom-scrollbar scrollbar-hide">
          ...
          <div className="p-5 md:p-8 border-t border-border/60 bg-card/10 flex flex-row gap-3">
            <Button type="button" variant="ghost" onClick={() => setOpen(false)} className="flex-1 h-12 md:h-14 rounded-xl md:rounded-2xl text-muted-foreground uppercase text-[11px] font-bold tracking-widest">Vazgeç</Button>
            <Button disabled={isPending} type="submit" className="flex-[2] h-12 md:h-14 rounded-xl md:rounded-2xl bg-blue-600 hover:bg-blue-500 text-white shadow-lg shadow-blue-600/20 gap-2 uppercase text-[11px] font-bold tracking-widest transition-all active:scale-95">
