...
export function ServiceStatusChart({ data }: ServiceStatusChartProps) {
  ...
  const chartData = data.length > 0 ? data : [
    { name: "VERİ YOK", value: 1, color: "#1f1f23" }
  ];
                ...
                if (active && payload && payload.length) {
                  return (
                    <div className="bg-[#141416] border border-white/10 p-3 rounded-xl shadow-2xl">
                      <p className="text-[10px] font-black uppercase tracking-widest mb-1" style={{ color: payload[0].payload.color }}>{payload[0].name}</p>
