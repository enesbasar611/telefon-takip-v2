                    ...
                    {step === "finance" && (
                        ...
                        >
                            <div className="text-center space-y-2">
                                ...
                                <p className="text-gray-400 text-sm md:text-base">Kasa, banka ve devir bakiyelerinizi tanımlayın.</p>
                            </div>

                            <div className="space-y-4">
                                {accounts.map((acc, index) => (
                                    <div key={acc.id} className="p-6 bg-white/[0.02] border border-white/10 rounded-[2rem] space-y-4 relative group">
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            <div className="space-y-2">
                                                <Label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Hesap Adı</Label>
                                                <Input
                                                    placeholder="Örn: Merkez Kasa, Ziraat Bankası"
                                                    value={acc.name}
                                                    onChange={e => setAccounts(accounts.map(a => a.id === acc.id ? { ...a, name: e.target.value } : a))}
                                                    className="h-12 bg-black/40 border-white/10 rounded-xl focus:border-white"
                                                />
                                            </div>
                                            <div className="space-y-2">
                                                <Label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Hesap Türü</Label>
                                                <Select
                                                    value={acc.type}
                                                    onValueChange={v => setAccounts(accounts.map(a => a.id === acc.id ? { ...a, type: v } : a))}
                                                >
                                                    <SelectTrigger className="h-12 bg-black/40 border-white/10 rounded-xl">
                                                        <SelectValue />
                                                    </SelectTrigger>
                                                    <SelectContent className="bg-black border-white/10 text-white p-2">
                                                        <SelectItem value="CASH">Nakit Kasa</SelectItem>
                                                        <SelectItem value="BANK">Banka Hesabı</SelectItem>
                                                        <SelectItem value="POS">POS Terminali</SelectItem>
                                                        <SelectItem value="CREDIT_CARD">Şirket Kredi Kartı</SelectItem>
                                                    </SelectContent>
                                                </Select>
                                            </div>
                                            <div className="space-y-2">
                                                <Label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Devir Bakiyesi (₺)</Label>
                                                <div className="relative">
                                                    <Input
                                                        type="number"
                                                        placeholder="0.00"
                                                        value={acc.balance}
                                                        onChange={e => setAccounts(accounts.map(a => a.id === acc.id ? { ...a, balance: parseFloat(e.target.value) || 0 } : a))}
                                                        className="h-12 bg-black/40 border-white/10 rounded-xl focus:border-white pr-10"
                                                    />
                                                    {index > 0 && (
                                                        <Button
                                                            size="icon"
                                                            ...
                                                            className="absolute right-1 top-1 h-10 w-10 text-rose-500/50 hover:text-rose-500 hover:bg-rose-500/10 rounded-lg"
                                                        >
                                        ...
                                        {acc.type === "CREDIT_CARD" && (
                                            <motion.div
                                                initial={{ opacity: 0, height: 0 }}
                                                animate={{ opacity: 1, height: "auto" }}
                                                className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-white/5"
                                            >
                                                <div className="space-y-2">
                                                    <Label className="text-[9px] font-bold text-indigo-400 uppercase tracking-widest pl-1">Kart Limiti (₺)</Label>
                                                    <Input
                                                        type="number"
