    ...
    <div className="flex flex-col gap-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">Orgelux Hub 2026</span>
          </div>
          <h1 className="text-4xl font-black tracking-tighter text-white uppercase italic">CİHAZ <span className="text-blue-500">MERKEZİ</span></h1>
      ...
      <div className="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-3">
        <div className="matte-card p-6 lg:p-8 rounded-[2rem] flex flex-col gap-6 relative overflow-hidden group">
            ...
            <div>
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">STOKTAKİ CİHAZLAR</p>
                <h3 className="text-4xl font-black text-white tracking-tighter">{devices.length} <span className="text-sm text-slate-600 font-bold ml-1">Ünite</span></h3>
            </div>
        </div>
        ...
        <div className="matte-card p-8 rounded-[2rem] flex flex-col gap-6 relative overflow-hidden group">
            ...
            <div>
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">BU AY SATILAN</p>
                <h3 className="text-4xl font-black text-white tracking-tighter">0 <span className="text-sm text-slate-600 font-bold ml-1">Ünite</span></h3>
            </div>
        </div>
        ...
        <div className="matte-card p-8 rounded-[2rem] flex flex-col gap-6 relative overflow-hidden group">
            ...
            <div>
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">GÜNCEL VARLIK DEĞERİ</p>
                <h3 className="text-4xl font-black text-white tracking-tighter">₺{devices.reduce((acc: number, d: any) => acc + Number(d.buyPrice), 0).toLocaleString('tr-TR')}</h3>
            </div>
        </div>
      </div>
      ...
      <div className="matte-card rounded-[2rem] overflow-hidden">
        <div className="p-6 lg:p-8 border-b border-slate-800/50 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900/20">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h2 className="text-sm font-black text-white uppercase tracking-widest">Envanter Arşivi</h2>
                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-tighter mt-0.5 italic">Gerçek zamanlı cihaz analizi</p>
                </div>
            </div>
        </div>
        ...
        <Table className="hidden lg:table">
          <TableHeader className="bg-slate-900/40">
            <TableRow className="border-slate-800/50 hover:bg-transparent">
              ...
              <TableHead className="py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Sınıflandırma</TableHead>
              <TableHead className="py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Durum / Kondisyon</TableHead>
              <TableHead className="py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Maliyet & Satış</TableHead>
          ...
          <TableBody>
            {devices.length === 0 ? (
              <TableRow>
                <TableCell colSpan={5} className="h-40 text-center text-slate-600 font-black uppercase text-xs italic tracking-widest">
                ...
                <TableRow key={device.id} className="border-slate-800/50 group hover:bg-slate-900/30 transition-colors">
                  <TableCell className="py-6 pl-8 font-medium">
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                           <span className="block text-sm font-black text-white uppercase tracking-tight group-hover:text-blue-400 transition-colors">{device.name}</span>
                           <span className="block text-[9px] text-slate-600 font-bold uppercase tracking-widest mt-0.5">IMEI: {device.deviceInfo?.imei || "Bilinmiyor"}</span>
                        </div>
                    </div>
                  </TableCell>
                  <TableCell>
                     <Badge variant="outline" className="bg-slate-900 border-slate-800 text-[9px] font-black text-slate-500 uppercase tracking-widest py-1.5 px-4 rounded-xl">
                        {device.category.name}
                     </Badge>
                  </TableCell>
                  <TableCell className="text-center">
                    <div className="flex flex-col items-center gap-1.5">
                        ...
                        <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Kondisyon: {device.deviceInfo?.cosmeticScore}/10</span>
                    </div>
                  </TableCell>
                  <TableCell className="text-right">
                    <div className="flex flex-col items-end">
                       <span className="text-lg font-black text-white italic tracking-tighter">₺{Number(device.sellPrice).toLocaleString('tr-TR')}</span>
                       <span className="text-[9px] text-slate-600 font-bold uppercase tracking-widest">Maliyet: ₺{Number(device.buyPrice).toLocaleString('tr-TR')}</span>
                    </div>
                  </TableCell>
