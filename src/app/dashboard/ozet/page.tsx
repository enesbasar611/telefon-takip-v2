...
export default async function DashboardOzetPage() {
  ...
  const stats = [
    ...
    { label: "TAMİR GELİRLERİ", value: statsData.todayRepairIncome, icon: Wrench, color: "text-cyan-500", bg: "bg-cyan-500/10", border: "border-cyan-500/10", trend: "+8%" },
    ...
    <div className="flex flex-col gap-8 pb-20 bg-[#0a0a0b] text-white min-h-screen p-8">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-4">
            <div className="h-12 w-12 rounded-2xl bg-cyan-500/10 flex items-center justify-center border border-cyan-500/20 shadow-cyan-sm">
                <LayoutDashboard className="h-6 w-6 text-cyan-500" />
            </div>
            <div>
                <h1 className="text-3xl font-black tracking-tighter uppercase">Komuta <span className="text-cyan-500">Merkezi</span></h1>
            ...
            <CardContent className="p-6 flex flex-col justify-between h-full relative z-10">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex flex-col items-end gap-1.5">
                    {stat.trend && (
                    <span className="text-[9px] font-black text-emerald-500 bg-emerald-500/10 px-2.5 py-1 rounded-lg border border-emerald-500/20 shadow-cyan-sm">
                        {stat.trend}
                    </span>
                    )}
                    {stat.badge && (
                    <span className={`text-[9px] font-black uppercase px-2.5 py-1 rounded-lg border shadow-cyan-sm ${stat.badge === 'URGENT' ? 'text-orange-500 bg-orange-500/10 border-orange-500/20' : 'text-rose-500 bg-rose-500/10 border-rose-500/20'}`}>
                        {stat.badge}
                    </span>
                    )}
                </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card className="bg-[#141416] border border-white/[0.03] shadow-2xl overflow-hidden group">
          <CardHeader className="flex flex-row items-center justify-between border-b border-white/[0.03] pb-6 bg-white/[0.01]">
            ...
            <div className="h-8 w-8 rounded-xl bg-blue-500/10 flex items-center justify-center border border-blue-500/20 shadow-cyan-sm">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="bg-[#141416] border border-white/[0.03] shadow-2xl overflow-hidden">
          <CardHeader className="flex flex-row items-center justify-between border-b border-white/[0.03] pb-6 bg-white/[0.01]">
            <div className="flex items-center gap-3">
                <div className="h-8 w-8 rounded-xl bg-emerald-500/10 flex items-center justify-center border border-emerald-500/20 shadow-cyan-sm">
            ...
            <Link href="/finans">
                <Button variant="ghost" className="text-[10px] font-black uppercase text-cyan-500 hover:bg-cyan-500/10 hover:text-cyan-400 group h-8 rounded-xl px-4 transition-all">
                  ...
                  {recentTransactions.map((t: any) => (
                    <tr key={t.id} className="text-sm group hover:bg-white/[0.01] transition-colors">
                      <td className="px-8 py-5">
                        <div className="font-black text-white text-xs uppercase tracking-tight group-hover:text-cyan-400 transition-colors">{t.sale?.customer?.name || "Hızlı Satış"}</div>
              ...
              <div key={ticket.id} className="flex items-center justify-between p-5 bg-white/[0.02] whisper-border border-white/5 rounded-3xl group hover:bg-white/[0.04] hover:border-white/10 transition-all shadow-xl">
                <div className="flex items-center gap-5">
                  <div className="h-14 w-14 rounded-2xl bg-white/[0.03] border border-white/5 flex items-center justify-center group-hover:scale-105 transition-transform relative">
                    ...
                    <div className="absolute -top-1 -right-1 h-3 w-3 rounded-full bg-cyan-500 animate-pulse border-2 border-[#141416] shadow-cyan-sm" />
                  </div>
                  <div>
                    <h4 className="font-black text-sm text-white uppercase tracking-tight group-hover:text-cyan-400 transition-colors">{ticket.deviceBrand} {ticket.deviceModel}</h4>
          ...
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
            {topProducts.map((product: any) => (
              <div key={product.id} className="flex flex-col gap-4 group">
                <div className="aspect-square rounded-[2rem] bg-white/[0.02] whisper-border border-white/5 flex items-center justify-center group-hover:bg-white/[0.04] group-hover:border-cyan-500/20 transition-all relative shadow-2xl overflow-hidden">
                   <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                   <Package className="h-16 w-16 text-gray-800 group-hover:text-cyan-500/40 group-hover:scale-110 transition-transform" />
                ...
                <div>
                  <h4 className="font-black text-sm truncate text-white uppercase tracking-tight group-hover:text-cyan-400 transition-colors">{product.name}</h4>
                  ...
                  <div className="flex items-center justify-between mt-3">
                    <span className="text-lg font-black text-cyan-500 tracking-tighter shadow-cyan-sm">₺{product.price.toLocaleString('tr-TR')}</span>
