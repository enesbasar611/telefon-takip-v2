        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-3">
                <h1 className="text-3xl font-black  ">{ticket.deviceBrand} <span className="text-blue-500">{ticket.deviceModel}</span></h1>
                <Badge variant="outline" className={`text-[10px] font-black px-4 py-1 rounded-xl whisper-border ${statusColors[ticket.status]}`}>
                    {statusLabels[ticket.status]}
                </Badge>
            </div>
            <p className="text-[10px] text-gray-500 font-bold   mt-1 italic">Servis Kaydı Analizi • #{ticket.ticketNumber}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
             <Button variant="ghost" className="text-[10px] font-black  text-blue-500 bg-blue-500/5 border border-blue-500/20 h-10 rounded-xl px-6 hover:bg-blue-500/10 transition-all ">
                PROFİLİ DÜZENLE
             </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
             ...
             <Card className="bg-[#141416] border-white/5 shadow-none group overflow-hidden relative">
                ...
                <CardHeader className="border-b border-white/[0.03] pb-4 bg-white/[0.01]">
                    <div className="flex items-center gap-3">
                        ...
                        <CardTitle className="text-xs font-black   text-white">Cihaz Bilgileri</CardTitle>
                    </div>
                </CardHeader>
                <CardContent className="p-6 space-y-4">
                    <div className="flex justify-between items-center border-b border-white/[0.02] pb-3">
                        <span className="text-[10px] font-bold text-gray-600  ">IMEI / SERİ NO</span>
                        <span className="text-xs font-black text-white">{ticket.imei || ticket.serialNumber || 'Bilinmiyor'}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-white/[0.02] pb-3">
                        <span className="text-[10px] font-bold text-gray-600  ">KOZMETİK DURUM</span>
                        <span className="text-xs font-black text-white ">{ticket.cosmeticCondition || 'Normal'}</span>
                    </div>
                    <div className="flex justify-between items-center">
                        <span className="text-[10px] font-bold text-gray-600  ">ARIZA TANIMI</span>
                        <span className="text-xs font-black text-blue-400  ">{ticket.problemDesc}</span>
                    </div>
             ...
             <Card className="bg-[#141416] border-white/5 shadow-none group overflow-hidden relative">
                ...
                <CardHeader className="border-b border-white/[0.03] pb-4 bg-white/[0.01]">
                    <div className="flex items-center gap-3">
                        ...
                        <CardTitle className="text-xs font-black   text-white">Müşteri Profili</CardTitle>
                    </div>
                </CardHeader>
                <CardContent className="p-6 space-y-4">
                    <div className="flex justify-between items-center border-b border-white/[0.02] pb-3">
                        <span className="text-[10px] font-bold text-gray-600  ">AD SOYAD</span>
                        <span className="text-xs font-black text-white ">{ticket.customer.name}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-white/[0.02] pb-3">
                        <span className="text-[10px] font-bold text-gray-600  ">İLETİŞİM</span>
                        <span className="text-xs font-black text-white">{ticket.customer.phone}</span>
                    </div>
                    <div className="flex justify-between items-center">
                        <span className="text-[10px] font-bold text-gray-600  ">TOPLAM İŞLEM</span>
          ...
          <Card className="bg-[#141416] border-white/5 shadow-none">
            <CardHeader className="border-b border-white/[0.03] pb-6 bg-white/[0.01]">
                <div className="flex items-center gap-3">
                    ...
                    <CardTitle className="text-sm font-black   text-white">Servis Yolculuğu</CardTitle>
                </div>
            </CardHeader>
                        ...
                        <div key={log.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
                            ...
                            <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-5 rounded-2xl whisper-border border-white/5 bg-white/[0.02] shadow-none group-hover:bg-white/[0.04] transition-all">
                                <div className="flex items-center justify-between space-x-2 mb-1">
                                    <div className="font-black text-white text-xs  ">{statusLabels[log.status]}</div>
        ...
        <div className="space-y-8">
            <Card className="bg-[#141416] border-white/5 shadow-none group overflow-hidden relative">
                ...
                <CardHeader className="border-b border-white/[0.03] pb-4 bg-white/[0.01]">
                    <div className="flex items-center gap-3">
                        ...
                        <CardTitle className="text-xs font-black   text-white">Finansal Özet</CardTitle>
                    </div>
                </CardHeader>
                <CardContent className="p-8 space-y-6">
                    <div className="text-center">
                        <p className="text-[10px] font-black text-gray-600   mb-2">TAHMİNİ ÜCRET</p>
                        <h2 className="text-4xl font-black text-white ">₺{Number(ticket.estimatedCost).toLocaleString('tr-TR')}</h2>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 rounded-2xl bg-white/[0.03] border border-white/5 text-center">
                            <p className="text-[8px] font-black text-gray-600  mb-1">ALINAN KAPORA</p>
                            <p className="text-sm font-black text-emerald-500">₺0</p>
                        </div>
                        <div className="p-4 rounded-2xl bg-white/[0.03] border border-white/5 text-center">
                            <p className="text-[8px] font-black text-gray-600  mb-1">KALAN TUTAR</p>
            ...
            <Card className="bg-[#141416] border-white/5 shadow-none group overflow-hidden relative">
                <CardHeader className="border-b border-white/[0.03] pb-4 bg-white/[0.01]">
                    <div className="flex items-center gap-3">
                        ...
                        <CardTitle className="text-xs font-black   text-white">Ekip & Operasyon</CardTitle>
                    </div>
                </CardHeader>
                <CardContent className="p-6 space-y-6">
                    <div className="flex items-center gap-4 p-4 rounded-2xl bg-white/[0.02] whisper-border border-white/5">
                        ...
                        <div>
                            <p className="text-[8px] font-black text-gray-600  ">GÖREVLİ TEKNİSYEN</p>
                            <p className="text-xs font-black text-white ">{ticket.technician?.name || 'HENÜZ ATANMADI'}</p>
                        </div>
                    </div>
                    <div className="space-y-3 pt-2">
                        <ServiceStatusUpdater ticketId={ticket.id} currentStatus={ticket.status} />
                        <Button variant="outline" className="w-full h-12 rounded-2xl border-white/5 bg-white/[0.03] text-white font-black   hover:bg-white/5 transition-all">
                           FİŞ YAZDIR
                        </Button>
                    </div>
                </CardContent>
            ...
            <div className="p-6 rounded-3xl whisper-border border-rose-500/10 bg-rose-500/5 flex items-start gap-4">
                ...
                <div>
                    <h4 className="text-[10px] font-black text-rose-500   mb-1">Sistem Uyarısı</h4>
