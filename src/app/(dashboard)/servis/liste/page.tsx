  ...
  return (
    <div className="flex flex-col gap-6 bg-background text-foreground min-h-screen lg:p-10 p-6 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <CreateServiceModal
          trigger={
            <Button className="gap-2 rounded-xl font-bold transition-all hover:scale-105 active:scale-95 shadow-lg shadow-primary/20">
      ...
      <Tabs defaultValue="active" className="w-full">
        <TabsList className="bg-muted/30 border border-border/40 rounded-2xl p-1.5 gap-1 h-auto mb-4 overflow-x-auto overflow-y-hidden max-w-full no-scrollbar">
          <TabsTrigger value="active" className="rounded-xl text-xs font-bold data-[state=active]:bg-background data-[state=active]:shadow-sm px-6 py-2.5 transition-all">
            Aktif ({counts.active})
          </TabsTrigger>
          <TabsTrigger value="ready" className="rounded-xl text-xs font-bold data-[state=active]:bg-background data-[state=active]:shadow-sm px-6 py-2.5 transition-all">
            Hazır ({counts.ready})
          </TabsTrigger>
          <TabsTrigger value="done" className="rounded-xl text-xs font-bold data-[state=active]:bg-background data-[state=active]:shadow-sm px-6 py-2.5 transition-all">
            Geçmiş ({counts.done})
          </TabsTrigger>
          <TabsTrigger value="all" className="rounded-xl text-xs font-bold data-[state=active]:bg-background data-[state=active]:shadow-sm px-6 py-2.5 transition-all">
            Tümü ({counts.all})
          </TabsTrigger>
        </TabsList>
