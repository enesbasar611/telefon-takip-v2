...
export default async function YazdirPage({ searchParams }: { searchParams: { id: string } }) {
  ...
  if (!ticket) {
    return <div className="p-8 text-center text-red-500  bg-white">Servis kaydı bulunamadı.</div>;
  }
  ...
  return (
    <div className="bg-white text-black p-4 w-[80mm] mx-auto min-h-screen font-sans text-[10px] leading-tight flex flex-col">
      ...
      <div className="text-center border-b-2 border-black pb-4 mb-4">
        ...
        <p className="text-[9px]  mb-2">{settings?.subtitle || "Mobil servis & teknik destek"}</p>
      ...
      <div className="flex justify-between items-center bg-black text-white px-2 py-2 mb-4">
        <span className=" text-[10px]">SERVİS FİŞİ</span>
      ...
      <div className="space-y-2 mb-4 border-b pb-4 border-black/10">
        <div className="grid grid-cols-2 gap-2">
          <div>
            <p className="text-[7px]  text-gray-500 mb-0.5">Müşteri bilgisi</p>
            <p className=" text-[11px] leading-none">{ticket.customer.name}</p>
            <p className="text-[9px]  mt-1">{ticket.customer.phone}</p>
          </div>
          <div className="text-right">
            <p className="text-[7px]  text-gray-500 mb-0.5">Kayıt tarihi</p>
            <p className=" text-[9px]">{format(new Date(ticket.createdAt), "dd.MM.yyyy", { locale: tr })}</p>
      ...
      <div className="mb-4">
        <div className="bg-gray-100 px-2 py-1.5 mb-2 rounded-sm">
          <p className="text-[8px]  text-center">CİHAZ VE ARIZA BİLGİLERİ</p>
        </div>
        <div className="space-y-1.5 px-1">
          <div className="flex justify-between items-baseline border-b border-dashed border-gray-200 pb-1">
            <span className="text-[8px]  text-gray-500">MARKA / MODEL:</span>
            <span className=" text-[10px]">{ticket.deviceBrand} {ticket.deviceModel}</span>
          </div>
          <div className="flex justify-between items-baseline border-b border-dashed border-gray-200 pb-1">
            <span className="text-[8px]  text-gray-500">IMEI / SERİ NO:</span>
            <span className=" text-[9px] select-all">{ticket.imei || ticket.serialNumber || "Yok"}</span>
          </div>
          <div className="mt-2">
            <p className="text-[8px]  text-gray-500 mb-1">BİLDİRİLEN ARIZA:</p>
      ...
      {ticket.usedParts && ticket.usedParts.length > 0 && (
        <div className="mb-6">
          <div className="bg-gray-100 px-2 py-1.5 mb-2 rounded-sm">
            <p className="text-[8px]  text-center">KULLANILAN PARÇALAR VE GARANTİ</p>
          </div>
            ...
            {ticket.usedParts.map((part: any, idx: number) => (
              <div key={idx} className="border-b border-gray-100 pb-2">
                <div className="flex justify-between items-start">
                  ...
                  <span className=" text-[9px] ml-4">₺{Number(part.unitPrice).toLocaleString('tr-TR')}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="flex flex-col items-center justify-center my-6 py-4 border-y border-black/5 bg-gray-50/50">
        ...
        <p className="text-[7px]  text-gray-400 mt-1">www.basarteknik.com/sorgula</p>
      </div>

      {/* Pricing & Summary */}
      <div className="mb-6">
        <div className="flex justify-between items-center border-t-2 border-black pt-3">
          <span className="text-[10px] ">TOPLAM TUTAR</span>
          <span className="text-xl ">₺{Number(ticket.actualCost > 0 ? ticket.actualCost : ticket.estimatedCost).toLocaleString('tr-TR')}</span>
        </div>
        <p className="text-[7px]  text-gray-400 text-right mt-0.5">Fiyatlara KDV dahildir.</p>
      </div>

      {/* Terms & Conditions */}
      {settings?.terms && (
        <div className="bg-gray-50 p-3 rounded-lg border border-gray-200 mb-6">
          <p className="text-[8px]  mb-1.5 border-b border-gray-200 pb-1">ÖNEMLİ ŞARTLAR</p>
      ...
      <div className="flex justify-between items-end px-4 mb-8">
        <div className="text-center">
          <p className="text-[7px]  mb-8">MÜŞTERİ İMZA</p>
          <div className="w-20 border-t border-black"></div>
        </div>
        <div className="text-center">
          <p className="text-[7px]  mb-8">TEKNİSYEN İMZA</p>
          <div className="w-20 border-t border-black"></div>
        </div>
      </div>
      ...
      <div className="mt-auto text-center opacity-40 grayscale pb-2">
        ...
        <p className="text-[6px] ">POWERED BY WEBFONE SOFTWARE SOLUTIONS</p>
      </div>
